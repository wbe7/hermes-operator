...
with metadata_route(get, evidence):
 b=check('baseline');private=['metadataStandin']+['target8080'+str(f) for f in [4,6]]+['target8081'+str(f) for f in [4,6]]+['neighbor8080'+str(f) for f in [4,6]]+['service'+str(f) for f in [4,6]]+['hostingNodeAPI'+str(f) for f in [4,6]]
 ...
 p.joinpath('verdict.json').write_text(json.dumps({'metadataStandin':'PASS: owned kind node exact-source DNAT to controlled neighbor:8081; not real cloud metadata','privateIPv4':'PASS','privateIPv6':'PASS','dnsTCPUDPv4v6':'PASS','exactExceptionsIPv4IPv6':'PASS','publicIPv4':'PASS','publicIPv6':'PASS' if public6 else 'not-run: unavailable in unrestricted baseline','restoration':'PASS'},indent=2))
