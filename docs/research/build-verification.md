...
- `go run golang.org/x/vuln/cmd/govulncheck@v1.1.4 ./...`, using Go 1.27.1 on the local darwin/arm64 target: exit 0, `No vulnerabilities found.` This did not establish Linux compatibility; see the CI correction below.
...
## CI scanner compatibility correction

The first [GitHub static job](https://github.com/wbe7/hermes-operator/actions/runs/35593427188/job/106312729640) passed Helm/generated/docs checks and `go vet`, then govulncheck v1.1.4 crashed in its bundled `golang.org/x/tools v0.29.0` SSA builder with `panic: unexpected expr: *ast.KeyValueExpr`. This was an analyzer crash, not a vulnerability finding.

With the same Go 1.27.1 toolchain and repository, a native scanner executable targeting `GOOS=linux GOARCH=amd64 CGO_ENABLED=0` reproduced that panic and exit 2. Changing only the scanner to [v1.8.0](https://pkg.go.dev/golang.org/x/vuln@v1.8.0/cmd/govulncheck), which bundles x/tools v0.50.0, produced exit 0 and `No vulnerabilities found.` CI now pins v1.8.0 and retains the full source scan and failure exit behavior. Runtime dependencies and the operator image are unchanged.

To reproduce the Linux-target scan from a different host architecture, first install the pinned scanner natively, then run it with the target build environment:

```bash
GOTOOLCHAIN=go1.27.1 GOBIN=/tmp/hermes-govulncheck go install golang.org/x/vuln/cmd/govulncheck@v1.8.0
GOOS=linux GOARCH=amd64 CGO_ENABLED=0 GOTOOLCHAIN=go1.27.1 /tmp/hermes-govulncheck/govulncheck ./...
```
