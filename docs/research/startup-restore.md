...
## Runtime evidence 2026-09-18

[Restricted official-image tests and live startup](runtime-verification.md) now verify the native SessionStore primary/mirror reset without session-ID/history loss, selected custom provider credential precedence, same-container restoration, and non-root gateway startup. The implementation uses surgical SQLite routing updates with fail-closed verification instead of a setter that may fallback silently. JSON mirror `_README` is metadata, not a session entry. Channel model/provider overrides are cleared while their personal prompts remain.

The heartbeat's epoch `start_time` and status record's `/proc` start ticks differ. Live tests required upstream runtime-status identity validation plus heartbeat epoch/process-creation validation and the local UNIX loop-tick socket; snapshot age alone is not liveness. Same-Pod Kubernetes and live user conversation acceptance remain separate.
