...
Для сброса smoke используйте новый пустой PVC, а прежний том сохраните. Сначала
остановите старую инсталляцию через `spec.suspend` или удалите CR и дождитесь
полного исчезновения его Pod. Только после этого запускайте новый gateway с тем
же Telegram token. Два polling-процесса одного бота конфликтуют; два независимо
работающих Telegram-агента требуют разных bot tokens.
...
Для возврата старых данных используйте прежний PVC явно, остановив другие его
потребители. Не удаляйте namespace, retained PVC или финализаторы ради сброса.
